...
<script type="text/javascript">
  ...
  // Query has to be lower case to comply with lowered DB columns names, e.g. vesselNameLower
  const query = urlParams && urlParams.toLowerCase();
  ...
  function createCard({port, vesselName}) {
    ...
    const card = `
      ...
      <div class="home-list-pop list-spac">
        <!--LISTINGS IMAGE-->
        <div class="col-md-3"> <img src="https://source.unsplash.com/collection/3882058/vessels" alt="" /> </div>
        <!--LISTINGS: CONTENT-->
        <div class="col-md-9 home-list-pop-desc inn-list-pop-desc"> <a href=""><h3>${vesselName}</h3></a>
          <h4>${port}</h4>
  ...
  $.ajax({
    ...
    success: function(result){
      if(result.data) {
        console.log('success ->', result.data.vessels)
        const vessels = result.data.vessels

        // Create the cards
        if (vessels.length > 0) {
          vessels.forEach((vessel) => createCard(vessel))
        } else {
          resultsTable.append(`<h2>We didn't find any results.</h2>`)
        }
      } else {
